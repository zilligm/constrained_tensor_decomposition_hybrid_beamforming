...
plot(data.SNR_dB,data.SmyPM,'-o','LineWidth',1.5); hold on; grid on
plot(data.SNR_dB,data.SmyEV,'-+','LineWidth',1.5);
...
%     axes('position',[ 0.2376    0.4253    0.1500    0.2000])
    ...
    box on % put box around new pair of axes
        ...
        plot(data.SNR_dB,data.SmyPM,'-o','LineWidth',1.5); hold on; grid on
        plot(data.SNR_dB,data.SmyEV,'-+','LineWidth',1.5);
    fig.CurrentAxes.YLim = [57.19 57.27];
    fig.CurrentAxes.XLim = [-0.008 0.008];
...
save = 1;
if save==1
    print(fig,'-depsc',strcat('Figures\PowerMethVsEigenvector_SE','.eps'));
    print(fig,'-dpdf',strcat('Figures\PowerMethVsEigenvector_SE','.pdf'));
end    
% close
