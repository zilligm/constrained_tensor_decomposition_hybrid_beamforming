function [ F, W,DATA] = myMUOFDMBeamformingRCD_eig( H, Ns, sigma2, Pt, rho)
    if length(size(H)) == 4
        [Nr,Nt,M,K] = size(H);
    else
        ...
        for s = 1:Ns
            for k = 1:K 
                ...
                while ((count < maxIte)&&(abs(delta(end)-delta(end-1))> epsilon))
                    ...
                    [EVw,evw] = svd(HresP);
                    w = (1/sqrt(Nr))*exp(1i*angle( EVw(:,find(diag(evw) == max(diag(evw)))) ));
                    ...
                    [EVf,evf] = svd(HresQ);
                    f = (1/sqrt(Nt))*exp(1i*angle( EVf(:,find(diag(evf) == max(diag(evf)))) ));      
